...
public class VectorClockTest extends AbstractVersionTest {
    ...
    public void create() {
        ...
        final List<VectorClockEntry> expected = Collections
                .singletonList(new VectorClockEntry(A, 1));
    ...
    public void incrementSameNode() {
        ...
        final List<VectorClockEntry> expected = Collections
                .singletonList(new VectorClockEntry(A, 2));
    ...
    public void incrementOtherNode() {
        final VectorClock a1b1 = (VectorClock) new VectorClock(A)
                .incrementFor(B);

        final List<VectorClockEntry> expected = Arrays
                .asList(new VectorClockEntry[] { new VectorClockEntry(A, 1),
                        new VectorClockEntry(B, 1) });
    ...
    public void incrementFirstOfManyNode() {
        final VectorClock a2b1c1 = (VectorClock) new VectorClock(A)
                .incrementFor(A).incrementFor(B).incrementFor(C)
                .incrementFor(A);

        final List<VectorClockEntry> expected = Arrays
                .asList(new VectorClockEntry[] { new VectorClockEntry(A, 3),
                        new VectorClockEntry(B, 1), new VectorClockEntry(C, 1) });
    ...
    public void incrementMiddleOfManyNode() {
        final VectorClock a1b2c1 = (VectorClock) new VectorClock(A)
                .incrementFor(B).incrementFor(C).incrementFor(B);

        final List<VectorClockEntry> expected = Arrays
                .asList(new VectorClockEntry[] { new VectorClockEntry(A, 1),
                        new VectorClockEntry(B, 2), new VectorClockEntry(C, 1) });
    ...
    public void isDescendantOfSame() {
        // same vector clocks should be equal but not descendant of
        final VectorClock a1 = new VectorClock(A);
    ...
    public void isDescendantOfMultipleNodes() {
        ...
        final VectorClock a1b1c1 = (VectorClock) a1.incrementFor(B)
                .incrementFor(C);
